...
`MusicControlButton` (`Sources/Core/MusicControlButton.swift`) is a **fixed
...
So it is replaced wholesale, not extended. `MusicControlButton` stays in the tree
only until every surface is converted, then it goes.
...
- **`MusicManager` still calls `AnchorViewCoordinator` at :1199-1201.** Two
  sneak-peek lines into Anchor's notch, which does not exist here. Delete them.
