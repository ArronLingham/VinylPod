#!/bin/bash
# Assert that no CADENCE_* environment hook survives into a Release build.
#
...
# string lives in Cadence.debug.dylib, not the main binary, so a naive check
# of Contents/MacOS/Cadence reports 0 for BOTH configurations and proves
# nothing. This script fails if the Debug build does not contain it.
set -euo pipefail
cd "$(dirname "$0")/.."

find_bundle() {
    find ~/Library/Developer/Xcode/DerivedData/Cadence-*/Build/Products/"$1" \
...
count_in() {  # every mach-o in the bundle, not just the main binary
    ...
    for f in "$bundle"/Contents/MacOS/*; do
        [ -f "$f" ] || continue
        total=$(( total + $(strings -a "$f" 2>/dev/null | grep -c 'CADENCE_' || true) ))
    done
    echo "$total"
}
...
d=$(count_in "$debug")
r=$(count_in "$release")
echo "Debug:   $d occurrence(s) of CADENCE_"
echo "Release: $r occurrence(s) of CADENCE_"

fail=0
[ "$d" -gt 0 ] || { echo "FAIL: the Debug build has no hook — this check is vacuous"; fail=1; }
[ "$r" -eq 0 ] || { echo "FAIL: a CADENCE_ hook survived into Release"; fail=1; }
